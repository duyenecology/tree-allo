...
gamma1_draws_pl_nou_h <- as.matrix(pl_nou_h[, "gamma[1]"])
gamma2_draws_pl_nou_h <- as.matrix(pl_nou_h[, "gamma[2]"])
sigma_draws_pl_nou_h <- as.matrix(pl_nou_h[, "sigma"])

num_sam_pl_nou_h <- nrow(gamma1_draws_pl_nou_h)
num_obs_pl_nou_h <- nrow(stan_data_lr_h$log_x)

log_y_pred_draws_pl_nou_h <- matrix(NA, nrow = num_sam_pl_nou_h, ncol = num_obs_pl_nou_h)

for (i in 1:num_sam_pl_nou_h) {
  log_y_pred_draws_pl_nou_h[i, ] <- gamma1_draws_pl_nou_h[i] + gamma2_draws_pl_nou_h[i] * stan_data_lr_h$log_x[, 2]
}

var_log_y_pred_pl_nou_h <- apply(log_y_pred_draws_pl_nou_h, 1, var)

var_res_pl_nou_h <- sigma_draws_pl_nou_h^2

bayes_R2_draws_pl_nou_h <- var_log_y_pred_pl_nou_h / (var_log_y_pred_pl_nou_h + var_res_pl_nou_h)

summary_bayes_R2_pl_nou_h <- summary(bayes_R2_draws_pl_nou_h)

summary_bayes_R2_pl_nou_h

# With wood density
pl_h <- tar_read(fit_lr2_draws_power_law1_h)
pl_h

gamma1_draws_pl_h <- as.matrix(pl_h[, "gamma[1,1]"])
gamma2_draws_pl_h <- as.matrix(pl_h[, "gamma[2,1]"])
sigma_draws_pl_h <- as.matrix(pl_h[, "sigma"])

num_sam_pl_h <- nrow(gamma1_draws_pl_h)
num_obs_pl_h <- nrow(stan_data_lr_h$log_x)

log_y_pred_draws_pl_h <- matrix(NA, nrow = num_sam_pl_h, ncol = num_obs_pl_h)

for (i in 1:num_sam_pl_h) {
  log_y_pred_draws_pl_h[i, ] <- gamma1_draws_pl_h[i] + gamma2_draws_pl_h[i] * stan_data_lr_h$log_x[, 2]
}

var_log_y_pred_pl_h <- apply(log_y_pred_draws_pl_h, 1, var)

var_res_pl_h <- sigma_draws_pl_h^2

bayes_R2_draws_pl_h <- var_log_y_pred_pl_h / (var_log_y_pred_pl_h + var_res_pl_h)

summary_bayes_R2_pl_h <- summary(bayes_R2_draws_pl_h)
summary_bayes_R2_pl_h
```
