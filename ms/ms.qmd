...
^1^ Yunnan Key Laboratory of Forest Ecosystem Stability and Global Change Response, Xishuangbanna Tropical Botanical Garden, Chinese Academy of Sciences, Menglun, Yunnan 666303, China
...
Our analysis combined tree allometry data from Tallo, which includes standardized measurements of DBH, height, and crown radius for 5,163 species across various biomes [@Jucker2022], with wood density data for 10,995 species from TRY version 6 [@Kattge2020] (see Supplementary Table S1 for further details).
Detailed information on data access and availability can be found in the Data availability section.
...
where $f(\cdot)$ predicts the expected natural log tree height or crown radius from observed DBH (and *vice versa*), $\pmb{\beta}_{j}=\{a_j, b_j, c_j, k_j\}$ denotes the vector of species-specific parameters, and $\sigma_y$ represents the standard deviation of the observation error.
...
Because these two models specifically describe saturating relationships (@eq-gMM; @eq-wb), their formulation does not allow for simply switching the roles of explanatory and response variables.
...
PSIS-LOO is an accurate and reliable approximation to standard leave-one-out cross-validation (LOO), a robust method for comparing models with different numbers of parameters [@Vehtari2017].
We utilized PSIS-LOO to calculate the LOO Information Criterion (LOOIC) for each model.
A model with a lower LOOIC is considered to have better predictive accuracy [@Vehtari2017].
...
where $\gamma_{0}$ and $\gamma_{1}$ are the global-level hyperparameters for intercept and slope, respectively, and $\sigma_{\beta}$ is the standard deviation of the species-specific parameters.
...
Estimates were considered significant when their 95\% CIs did not overlap with zero.
...
Codes are made available on Zenodo (https://zenodo.org) and GitHub (<https://github.com/duyenecology/tree-allo>).
Stan code is provided in Supplementary Section S2.
...
In gymnosperms, we found a positive association between wood density and the exponent parameter $b$, whereas neither the asymptote parameter $a$ nor the scaling constant $k$ showed a significant relationship with this functional trait (@fig-wood1 i, j, k; @fig-wood2 c).
...
AGB estimates based on the Weibull model ($\text{AGB}_\text{H-WB}$) showed the highest predictive accuracy based on both the lowest root mean square error (RMSE) of `r sprintf("%.3g", data_metrics$metrics_agb_ang$rmse_wb_h)` Mg and the smallest bias of `r sprintf("%.3g", data_metrics$metrics_agb_ang$bias_wb_h * 100)`% (@fig-agb a).
...
Our results supported a saturating H-DBH relationship in both angiosperm and gymnosperm species, consistent with theoretical expectations [@Falster2003; @Niklas2007] and empirical observations [e.g., @Feldpausch2011; @MartinezCano2019; @Song2024].
...
This asymptotic pattern likely arises from structural constraints in gymnosperms' growth forms that favor vertical over lateral expansion [e.g., @Lines2012; @Hulshof2015; @Jucker2024], an adaptation that can mitigate risks such as branch breakage under heavy snow loads [e.g., @Mikola1938; @Loehle2016; @Tavankar2019].
...
Our analysis indicated distinct patterns in which wood density has a greater influence on interspecific variation in angiosperms than in gymnosperms.
...
MK was supported by
the Xingdian Talent Support Program of Yunnan Province,
...
The tree allometry data is avaiable from tallo [@Jucker2022, https://doi.org/10.5281/zenodo.6637599], and
the wood density data are available on request from TRY version 6  [@Kattge2020, <https://www.try-db.org/TryWeb/Home.php>].
All codes and scripts used for analysis are available on Zenodo (https://zenodo.org) and GitHub (<https://github.com/duyenecology/tree-allo>).
